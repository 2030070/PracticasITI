...
<div class="container mx-auto px-4">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="col-span-1 md:col-span-2">
            <div class="">
                <form action="{{ route('categorias.update_imagen', $categoria->id) }}" method="POST" enctype="multipart/form-data" class="dropzone" id="dropzone" style="border: 4px solid; border-radius: 20px; border-image: linear-gradient(to right, #d77cd7, #3B82F6); border-image-slice: 1;">
                    @csrf
                    @method('PUT')
                </form>
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    <form action="{{ route('categorias.update', $categoria->id) }}" method="POST" novalidate>
                        ...
                        <div class="mb-5">
                            <input name="imagen" type="hidden" value="{{$categoria->imagen}}" />
                            @error('imagen')
                        ...
                        <div class="mb-4">
                          ...
                          <textarea name="descripcion" id="descripcion" required
                              ...
                              text-sm leading-5.6 ease block w-full appearance-none rounded-lg border-2 border-blue-500 bg-white 
                              bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 
                              focus:border-fuchsia-300 focus:outline-none">{{ $categoria->descripcion }}</textarea>
                      </div>
