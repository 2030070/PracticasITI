        ...
        <form action="{{ route('ventas.store') }}" method="POST">
            ...
            <div class="w-1/2 ml-2">
              ...
              <select name="nombre_cliente" id="nombre_cliente" class="focus:shadow-primary-outline dark:text-white/80 text-sm leading-5.6 ease block w-full appearance-none rounded-lg border-2 border-blue-500 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none">
                ...
                @foreach($clientes as $cliente)
                  <option value="{{ $cliente->nombre }}" {{ old('nombre_cliente') == $cliente->nombre ? 'selected' : '' }}>{{ $cliente->nombre }}</option>
                @endforeach
              </select>
          ...
          <div class="mb-4 flex">
            <div class="w-1/2 mr-2">
                ...
                <select name="pago" id="pago" class="focus:shadow-primary-outline dark:text-white/80 text-sm leading-5.6 ease block w-full appearance-none rounded-lg border-2 border-blue-500 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" placeholder="Seleccione el estado de pago">
                  ...
                  <option value="hecho" {{ old('pago') == 'hecho' ? 'selected' : '' }}>Hecho</option>
                  <option value="pendiente" {{ old('pago') == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                </select>
                @error('pago')
                  <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="w-1/2 ml-2">
              ...
              <input type="number" name="total" id="total" class="focus:shadow-primary-outline dark:text-white/80 text-sm leading-5.6 ease block w-full appearance-none rounded-lg border-2 border-blue-500 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" step="0.01" min="0" placeholder="Ingrese el total" value="{{ old('total') }}">
              @error('total')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
              @enderror
            </div>        
          ...
          <div class="mb-4 flex">
              <div class="w-1/2 mr-2">
                  ...
                  <input type="number" name="pago_parcial" id="pago_parcial" class="focus:shadow-primary-outline dark:text-white/80 text-sm leading-5.6 ease block w-full appearance-none rounded-lg border-2 border-blue-500 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" step="0.01" min="0" placeholder="Ingrese el pago parcial" value="{{ old('pago_parcial') }}">
                  @error('pago_parcial')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                  @enderror
              </div>
              <div class="w-1/2 ml-2">
                ...
                <input type="number" name="pago_pendiente" id="pago_pendiente" class="focus:shadow-primary-outline dark:text-white/80 text-sm leading-5.6 ease block w-full appearance-none rounded-lg border-2 border-blue-500 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" step="0.01" min="0" placeholder="Ingrese el pago pendiente" value="{{ old('pago_pendiente') }}">
