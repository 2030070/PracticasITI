...
@section('nav')
    <nav class="flex justify-center items-center mb-4 bg-blue-500">
        ...
        <div class="flex items-center ml-6" title="Registrar">
            <a href="{{ route('facturas.create')}}" class="flex items-center text-white">
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="#FFFFFF" viewBox="0 0 256 256">
...
@section('contenido')
    <div class="my-4 flex justify-end space-x-2">
        <button onclick="exportToPDF('facturas')" class="inline-block px-2 py-1 rounded-lg font-bold text-sm text-white bg-red-600 hover:bg-red-700 transition-colors">
            Exportar a PDF
        </button>

        <button onclick="exportToExcel('facturas')" class="inline-block px-2 py-1 rounded-lg font-bold text-sm text-white bg-green-600 hover:bg-green-700 transition-colors">
            Exportar a Excel
        </button>
    </div>
